...
<div class="bg-white shadow rounded-lg p-6 max-w-3xl">
    <form action="/roles/create" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" required class="mt-1 block w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        </div>

        <div class="mb-6">
            <label class="block text-sm font-medium text-gray-700">Description</label>
            <input type="text" name="description" class="mt-1 block w-full border border-gray-300 rounded-md py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        </div>
                ...
                <?php foreach ($permissions as $perm): ?>
                    <div class="flex items-start">
                        <div class="flex items-center h-5">
                            <input id="perm_<?= $perm['id'] ?>" name="permissions[]" value="<?= $perm['id'] ?>" type="checkbox" class="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded">
                        </div>
        ...
        <div class="flex justify-end space-x-3">
            ...
            <button type="submit" class="bg-indigo-600 py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Save Role</button>
        </div>
    </form>
</div>
